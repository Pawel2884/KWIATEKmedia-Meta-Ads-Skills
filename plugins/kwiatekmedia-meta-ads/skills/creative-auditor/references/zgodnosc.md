<!-- WYGENEROWANE z shared/zgodnosc.md przez scripts/build.py. Nie edytuj tutaj, edytuj shared/. -->
# Zgodność: polityki Meta i polskie prawo reklamy

To przegląd na potrzeby tworzenia kreacji, nie porada prawna. W sprawach granicznych (medycyna, finanse, prawo, finansowanie OZE) poleć konsultację z prawnikiem klienta. Przepisy oznaczone „do weryfikacji” mogły się zmienić.

## Spis treści
1. Checklista dla każdej reklamy
2. Cechy osobiste odbiorcy (polityka Meta)
3. Moduły branżowe (według flag z Karty Oferty)
4. Formularz i dane osobowe
5. AI w kreacjach
6. Jak przepisywać ryzykowne zdania

## 1. Checklista dla każdej reklamy

Polityki Meta (A):
- [ ] Brak twierdzeń i pytań o cechy odbiorcy (sekcja 2).
- [ ] Brak clickbaitu: ukrywania informacji, żeby wymusić klik, sensacyjnego języka, obietnic bez pokrycia.
- [ ] Brak engagement bait („skomentuj”, „oznacz znajomego”).
- [ ] Brak fałszywych elementów interfejsu (przycisk play na statyce, „nowa wiadomość”).
- [ ] Brak treści szokujących i wulgaryzmów, także maskowanych symbolami.
- [ ] Brak wizerunku znanych osób bez zgody.
- [ ] Poprawna gramatyka i interpunkcja (Meta wymienia ją w checkliście polityk).
- [ ] Obietnica reklamy zgodna ze stroną lub formularzem.
- [ ] Brak treści politycznych i społecznych (w UE reklamy o kwestiach społecznych, wyborach i polityce są niedozwolone). Nie wplataj polityki w hook („rachunki przez rząd”).

Polskie prawo (A, przegląd):
- [ ] Każde twierdzenie faktograficzne prawdziwe i możliwe do udowodnienia („nr 1”, „najtańszy”, „najszybszy” tylko z dowodem). Ustawa o przeciwdziałaniu nieuczciwym praktykom rynkowym (UoPNPR) art. 5; ustawa o zwalczaniu nieuczciwej konkurencji art. 16.
- [ ] Brak fałszywej pilności i ograniczonej dostępności (UoPNPR art. 7 pkt 7).
- [ ] „Bezpłatny”, „gratis”, „za darmo” tylko, gdy klient nie płaci nic poza kosztem odpowiedzi lub odbioru (art. 7 pkt 20).
- [ ] Certyfikaty, znaki jakości, „autoryzowany partner”, logo instytucji tylko z uprawnieniem (art. 7 pkt 2 i 4).
- [ ] Opinie prawdziwe i niezniekształcone. Opłacony twórca lub aktor nie udaje zwykłego klienta (art. 7 pkt 22, 25, 26).
- [ ] Brak straszenia nieuczciwym ryzykiem (art. 7 pkt 12).
- [ ] Prawa ustawowe (np. zwrot w 14 dni, rękojmia) nie są przedstawiane jako wyróżnik oferty (art. 7 pkt 10).
- [ ] Reklama porównawcza rzetelna i sprawdzalna, bez dyskredytowania nazwanych konkurentów (UZNK art. 16 ust. 3).
- [ ] Ceny dla konsumentów brutto; w B2B zaznacz „netto”.
- [ ] Obniżka ceny (towar lub usługa): obok informacja o najniższej cenie z 30 dni przed obniżką (ustawa o informowaniu o cenach, dyrektywa Omnibus).
- [ ] Obietnice efektu lub oszczędności tylko z warunkami i założeniami.

## 2. Cechy osobiste odbiorcy

Meta zabrania twierdzeń i sugestii, że odbiorca ma daną cechę. Obejmuje to także pytania o nią. „Ty” i „Twój” są dozwolone, jeśli nie łączą się z zakazaną cechą.

Cechy: rasa, pochodzenie, religia, wiek, płeć, orientacja, tożsamość płciowa, sytuacja rodzinna, niepełnosprawność, stan zdrowia fizycznego i psychicznego, trudna sytuacja finansowa, karalność, członkostwo w związkach zawodowych, poglądy, imię.

Przykłady naruszeń wg Meta: „Are you bankrupt? Our firm has solutions.”, „Have you been diagnosed with cancer?”, „If you are tired of dealing with depression…”, „…people like you who just turned 62”.

Przepisywanie (zasada: mów o usłudze, sytuacji zewnętrznej, miejscu albo roli, nie o cesze osoby):
| Naruszenie | Bezpieczniej |
|---|---|
| „Masz nadwagę?” | „Program redukcji masy ciała z dietetykiem” |
| „Masz cukrzycę? Zmierz…” | „Glukometr z aplikacją: jak działa pomiar” |
| „Po 60-tce bolą Cię kolana?” | „Rehabilitacja stawu kolanowego w Lublinie” |
| „Masz zły BIK?” | „Kredyt a historia w BIK: co sprawdzamy na pierwszej rozmowie” (i tak: moduł kredytowy) |
| „Jesteś po rozwodzie?” | „Podział majątku: jak wygląda pierwsza konsultacja” |
| „Zanim weźmiesz kolejną chwilówkę…” | „Kolejna pożyczka na spłatę poprzedniej. Kiedy lepszą drogą jest upadłość konsumencka?” |
| „Boli Cię kark po pracy?” | „Ból karku u osób pracujących przy biurku. Tym zajmujemy się w gabinecie przy Hetmańskiej.” |

Uwaga: długo działające polskie reklamy z pytaniami o długi lub zdrowie nie dowodzą, że to bezpieczne. Egzekwowanie jest nierówne, a naruszenia obniżają jakość konta.

## 3. Moduły branżowe

**ZDROWIE (gabinety, fizjoterapia, stomatologia, estetyka, dietetyka, psychoterapia)**
- Meta: bez negatywnej autopercepcji i zawstydzania wyglądem, bez zbliżeń „problemowych” partii ciała, bez obietnic konkretnego efektu w określonym czasie bez zastrzeżeń, bez obietnic wyleczenia chorób nieuleczalnych. Zabiegi estetyczne, odchudzanie, przed i po: targetowanie 18+.
- Prawo: podmiot leczniczy informuje o świadczeniach, ale treść i forma informacji nie mogą mieć cech reklamy (ustawa o działalności leczniczej art. 14). Od 31.10.2019 ten sam zakaz obejmuje wprost fizjoterapeutów (gabinety i praktyki indywidualne); Kodeks Etyki Zawodowej Fizjoterapeuty (2022) dopuszcza tylko informowanie.
- Styl informacyjny (ZDROWIE zawsze w tym trybie): zakres usługi, dla kogo, przebieg wizyty, czas, cena, miejsce, termin, jak się umówić, kwalifikacje. Bez superlatyw („najlepszy”), obietnic efektu, zachęt i nakłaniania („nie czekaj”, „zadbaj o siebie już dziś”), presji czasu, promocji handlowych, porównań. Opinie pacjentów w reklamie: ostrożnie, najlepiej wcale (mogą być uznane za zachętę).
- Kodeks Etyki Lekarskiej: od 1.01.2025 obowiązuje nowa wersja, która dopuszcza informowanie o usługach zgodnie z zasadami etyki (stary art. 63 z ogólnym zakazem reklamy nie obowiązuje). Art. 14 ustawy o działalności leczniczej nadal wymaga, by informacja nie miała cech reklamy.
- Formularz: bez pytań o diagnozy i stan zdrowia. Pytaj o usługę („czego dotyczy wizyta: kręgosłup, uraz sportowy, rehabilitacja po zabiegu”), preferowany termin i porę kontaktu.
- Nazwy usług prostym językiem pacjenta („ból dolnej części pleców”, „ból karku”), nie terminologią („odcinek lędźwiowy i szyjny”).
- Twarz specjalisty: może informować o usłudze; nie może „polecać” wyrobu medycznego.

**WYROB-MEDYCZNY**
- Bez wizerunku osób wykonujących zawody medyczne (ani udających je) prezentujących wyrób.
- Ostrzeżenie „To jest wyrób medyczny. Używaj go zgodnie z instrukcją używania lub etykietą.” na grafice co najmniej ok. 10% powierzchni u dołu, w wideo co najmniej ok. 15% i lektor. Kary do 2 mln zł (ustawa o wyrobach medycznych, rozporządzenie MZ z 2023; zakres dla reklam zabiegów sporny).

**FINANSE-KREDYT (kredyt, raty, leasing konsumencki, finansowanie zakupu)**
- Meta: Special Ad Category „kredyt” (Polska na liście). Brak targetowania po wieku, płci, kodach, lookalike. Kreacja i formularz muszą kwalifikować.
- Prawo: w każdej reklamie kredytu konsumenckiego RRSO. Jeśli pojawia się jakikolwiek koszt (rata, oprocentowanie, „0%”), pełny reprezentatywny przykład. Pośrednik podaje zakres umocowania. Dyrektywa CCD2 (stosowanie od 20.11.2026) wymaga ostrzeżenia, że pożyczanie kosztuje, ale na wrzesień 2026 Polska nie ma ustawy wdrażającej (projekt wycofany 22.05.2026). Nie podawaj żadnego brzmienia ostrzeżenia jako obowiązującego. Oznacz: `ZGODNOŚĆ: sprawdzić aktualny stan CCD2 u prawnika klienta`.
- Bez danych do RRSO: nie wspominaj o ratach.

**PRAWO (kancelarie, radcy, adwokaci)**
- Radca prawny: Kodeks Etyki Radcy Prawnego art. 31 daje prawo do „informowania o wykonywaniu zawodu” (każda komunikacja promująca radcę lub kancelarię). Informowanie musi być zgodne z zasadami etyki: rzetelne, bez wprowadzania w błąd, bez obietnic wyniku, bez porównań z innymi prawnikami, bez ujawniania klientów bez zgody, bez form natarczywych.
- Adwokat: Zbiór Zasad Etyki Adwokackiej § 23 zakazuje reklamy; dopuszcza „informację handlową”, która nie może być porównawcza, nierzetelna, nachalna, wywoływać nieuzasadnionych oczekiwań co do wyniku sprawy, odwoływać się do emocji ani wykorzystywać naiwności lub trudnego położenia odbiorcy.
- W praktyce dla obu: styl informacyjny (zakres spraw, przebieg współpracy, koszty, doświadczenie liczbowe, sposób kontaktu, poufność), spokojny ton, bez presji, bez straszenia, bez „wygraj”, „odzyskaj”, „pozbądź się długów” jako obietnicy wyniku.
- Firmy doradcze, które nie są kancelarią radcy lub adwokata, nie podlegają kodeksom zawodowym, ale obowiązuje je UoPNPR.
- Tematy finansowe (długi, upadłość) łączą się z zakazem cech osobistych Meta.
- Formularz przy długach i upadłości: nie pytasz o etap problemu („mam wezwanie”, „mam komornika”, „ile mam długów”), bo to pytanie o sytuację finansową. Pytasz o usługę i kontakt: czego dotyczy rozmowa (upadłość osoby prywatnej, upadłość działalności, inna sprawa), kiedy możesz rozmawiać, preferowana pora telefonu.

**NIERUCHOMOSCI, PRACA**
- Meta: Special Ad Category (Polska na liście). Kreacja przejmuje targetowanie, bez dyskryminujących sformułowań (wiek, płeć, rodzina, pochodzenie).

**OZE-EKO**
- Dyrektywa 2024/825 stosowana od 27.09.2026 (w Polsce projekt ustawy UC111 w toku na wrzesień 2026): bez ogólnych twierdzeń środowiskowych („eko”, „zielony”, „przyjazny środowisku”, „neutralny klimatycznie”) bez wykazanej, uznanej doskonałej efektywności. Konkretne parametry zamiast etykiet. Stosuj już teraz, niezależnie od stanu wdrożenia.
- Obietnice oszczędności („rachunek 0 zł”) tylko z warunkami i założeniami.

**DOTACJE**
- Nie sugeruj, że firma działa w imieniu instytucji albo ma jej „akredytację”, jeśli tak nie jest (UoPNPR art. 7 pkt 4). Nazwa programu może paść jako informacja („pomagamy złożyć wniosek do programu X”).
- Nie obiecuj kwoty dotacji, jeśli zależy od warunków odbiorcy.

**PROMOCJA-CENOWA**
- Obniżka: najniższa cena z 30 dni. „Darmowa dostawa” tylko bez ukrytych opłat. Fałszywe liczniki zakazane.

**B2B**
- Ceny netto oznaczone. Certyfikaty i logotypy klientów tylko za zgodą. UZNK obowiązuje także w B2B.

## 4. Formularz i dane osobowe

- Link do polityki prywatności z klauzulą informacyjną (wymaga też Meta; nie może to być PDF).
- Cel kontaktu opisany („prosisz o kontakt telefoniczny w sprawie wyceny”).
- Zgody marketingowe osobne, nieobowiązkowe, niezaznaczone domyślnie.
- Minimalizacja danych: tylko pytania potrzebne do kwalifikacji.
- Bez danych szczególnych (zdrowie) i bez zakazanych pytań Meta (finanse osobiste, zdrowie, ubezpieczenia, karalność).

## 5. AI w kreacjach

- AI Act art. 50 (stosowanie od 2.08.2026): realistyczne obrazy, wideo lub głos, które można wziąć za prawdziwe (osoby, miejsca, zdarzenia), wymagają informacji o sztucznym wygenerowaniu.
- Nigdy nie przedstawiaj wygenerowanych osób jako prawdziwych klientów, pacjentów, ekspertów ani wygenerowanych „realizacji” jako rzeczywistych.
- Awatar AI lub klon głosu nie udaje klienta ani pracownika. Klon właściciela tylko za pisemną zgodą i z oznaczeniem.
- Wizualizacje oznaczaj jako „wizualizacja”.
- Współpraca z twórcą: oznaczenie dwupoziomowe wg rekomendacji Prezesa UOKiK (2022): funkcja platformy („Płatne partnerstwo”) i ręcznie na początku opisu, grafiki lub nagrania, po polsku („materiał reklamowy”, „reklama”, „współpraca reklamowa z [marka]”).
- Ujawnienie, że reklama lub obraz powstały z AI, obniża zaufanie i zamiar zakupu (B/C: badania 2025–2026, silniej w wideo i przy drogich decyzjach). Wniosek: prawdziwe zdjęcia i nagrania klienta mają przewagę; AI do tła, wizualizacji i konceptów.

## 6. Jak przepisywać ryzykowne zdania

1. Zamień cechę osoby na usługę, sytuację zewnętrzną, miejsce lub rolę.
2. Zamień obietnicę efektu na opis procesu i warunków.
3. Zamień superlatyw na sprawdzalny fakt z Karty Oferty albo usuń.
4. Zamień pilność na prawdziwy termin z datą albo usuń.
5. Gdy nie masz pewności, oznacz: `ZGODNOŚĆ: do sprawdzenia przez prawnika klienta (powód)`.
